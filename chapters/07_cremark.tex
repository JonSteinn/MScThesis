...
Unlike the translation method in Wood and Zeilberger \cite{wood_zeilberger}, our automated bijection does not require any previous mathematical work, e.g., algebraic proof. It only depend on sufficient implementations of strategies for domains.

As we discussed in \SectionRef{s:fw}, the search is done from a universe that is expended with a completely different goal in mind. We believe this to be a limiting factor of finding bijections. On the contrary, its strength is that it can find a bijection without any supplied information, given that a domain has adequate strategies.
