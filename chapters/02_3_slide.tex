Sliding is a generalization of the sliding lemma described in Babson and West \cite[Lemma 2.2]{slide}.
...
An example for $3594^{(0,0)}62^{(1,0)}718^{(2,0)}$ can be seen in \FigureRef{fig:slide_gp_example}. This mapping is also outlined in Algorithm~\ref{alg:slidealg}\footnote{Every algorithm in this thesis are accompanied by a Python implementation.}.
