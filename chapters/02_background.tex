...
\end{align*}
A more detailed definition can be found in \authcite{albert2005insertion}. Given a permutation class $\Av{\mathcal{B}}$, the set of insertion encodings of all its permutation make up the language of the class. If this language is regular\footnote{Recognized by a finite automaton.} we say that the class is \emph{regular insertion encodable}. TileScope was shown to be able to find specifications for all regular insertion encodable classes by \citeauthor{BeanPhd:phd} \cite[Theorem  5.1.3]{BeanPhd:phd}.
